        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                    ...
                    <p class="text-2xl font-bold text-blue-600">{{ $todayScans }}</p>
                </div>
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $errorScans }}</p>
                </div>
            </div>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach($scans as $scan)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 text-sm font-mono text-gray-900">{{ substr($scan->barcode, 0, 10) }}...</td>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-600">{{ $scan->scan_location }}</td>
                                        <td class="px-6 py-4 text-sm">
                                            <span class="inline-block px-2 py-1 text-xs font-medium rounded
                                                @if($scan->status === 'success') bg-green-100 text-green-800
                                                @elseif($scan->status === 'error') bg-red-100 text-red-800
